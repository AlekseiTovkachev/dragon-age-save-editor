...
Run them against a copied or restorable DAO-family save:

```powershell
$env:DAO_SAVE = "C:\path\to\savegame.das"
npm run ingame-test -- combo.spec.ts
```

The suite is forced to one worker because all tests mutate the same `DAO_SAVE` and use one sidecar document.
