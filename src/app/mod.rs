...
use crate::validate::{validate_gff, ValidationCode, ValidationFinding, ValidationReport, ValidationSeverity};
...
pub enum SaveCommand {
    ...
    GetSummary,
    GetDocumentAssets,
    GetCharacter { target: CharacterTargetDto },
    ListAvailableAbilities { list: AbilityListKindDto },
    ListAvailableItemProperties,
    ListCharacters,
    ListBackpackItems,
    ListEquipmentItems { target: CharacterTargetDto },
    SetMoney { money: u32 },
    PatchCoreStats {
    ...
    SetLevel { target: CharacterTargetDto, level: u32 },
    SetApproval { target: CharacterTargetDto, approval: i32 },
    ReplaceAbilityList {
        target: CharacterTargetDto,
    ...
    RemoveBackpackItem { index: usize },
    ReplaceBackpackItem {
        index: usize,
    ...
    SaveAs { output_path: String },
}

#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(tag = "result", rename_all = "snake_case")]
pub enum SaveCommandResult {
    Validation { report: ValidationReportDto },
    Summary { summary: SaveSummaryDto },
    DocumentAssets { assets: DocumentAssetsDto },
    AvailableAbilities { list: AbilityListKindDto, abilities: Vec<AbilityDto> },
    AvailableItemProperties { properties: Vec<SelectableItemPropertyDto> },
    Characters { characters: Vec<CharacterSummaryDto> },
    Items { items: Vec<IndexedItemDto> },
    Character { target: CharacterTargetDto, character: CharacterDto },
    Item { container: InventoryContainerDto, index: usize, item: ItemDto },
    Saved { output_path: String, summary: SaveSummaryDto },
}
...
impl SaveDocument {
    ...
    pub fn from_gff(source_path: impl Into<PathBuf>, raw: GffFile) -> Result<Self, CommandError> {
        ...
        let lookup = resolve_game_data_path()
            ...
            .transpose()?;
        let editor_result = SaveEditor::from_gff_with_lookup(
            raw.clone(),
            lookup.as_ref().map(|db| db as _),
            None,
        );
    ...
    pub fn execute(&mut self, command: SaveCommand) -> Result<SaveCommandResult, CommandError> {
        match command {
            ...
            SaveCommand::SetMoney { money } => {
                self.editor_mut()?.set_money(money).map_err(CommandError::from)?;
                self.dirty = true;
    ...
    fn editor(&self) -> Result<&SaveEditor, CommandError> {
        self.editor
            .as_ref()
            .ok_or_else(|| self.load_error.clone().unwrap_or_else(|| CommandError {
    ...
    fn screenshot_data_url(&self) -> Result<Option<String>, CommandError> {
        ...
        let mut file = File::open(&screenshot_path).map_err(|err| CommandError {
            code: CommandErrorCode::Io,
            message: format!("failed to open screenshot {}: {err}", screenshot_path.display()),
        })?;
        let dds = Dds::read(&mut file).map_err(|err| CommandError {
            code: CommandErrorCode::Io,
            message: format!("failed to read DDS screenshot {}: {err}", screenshot_path.display()),
        })?;
...
impl From<EditError> for CommandError {
    fn from(value: EditError) -> Self {
        let code = match value {
            ...
            EditError::InvalidPropertyArrayParity { .. } => CommandErrorCode::InvalidPropertyArrayParity,
...
mod tests {
    ...
    fn save_as_writes_new_file_and_keeps_original_unchanged() {
        ...
        match response {
            SaveCommandResult::Saved { output_path, summary } => {
    ...
    fn validate_command_works_even_when_editor_cannot_be_built() {
        ...
        let error = document.execute(SaveCommand::ListBackpackItems).unwrap_err();
