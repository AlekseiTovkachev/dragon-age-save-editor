...
use crate::gff4::fields::{
    ITEM_COST, ITEM_STACKSIZE, OBJECT_ID, SAVEGAME_ITEM_LEVEL, SAVEGAME_ITEM_MATERIALTYPE,
...
pub(super) fn apply_item_metadata_patch_to_struct(
    ...
    if let Some(item_level) = patch.item_level {
        if let Some(value) = item.get_mut(SAVEGAME_ITEM_LEVEL) {
            set_signed_numeric_value(value, item_level as i32, "item.SAVEGAME_ITEM_LEVEL")?;
        }
    }
    Ok(())
}
