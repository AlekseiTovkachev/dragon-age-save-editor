...
class LazyList(object):
    ...
    def __getitem__(self, i):
        ...
        if isinstance(i, slice):
            ...
            value = [v() if isinstance(v, partial) else v for v in value]
            self._list[i] = value
...
class LazyGFF4(object):
    def __init__(self, f, preload=False):
        ...
        structrange = range(len(headerstructs))
            ...
            if issubclass(kind, List):
                ...
                if (elem_type, kind.indirect) in lists:
                    ...
                    kind = type(
                        kind.__name__ + "Lazy",
                        (LazyList, kind),
                        dict(__slots__=(), elem_type=elem_type),
                    )
        ...
        def make_structure(i, root=False):
            ...
            if i not in structs:
                ...
                struct = type(
                    oldstruct.__name__ + "Lazy",
                    (LazyStructure, oldstruct),
                    dict(__slots__=()),
                )
        ...
        self._header = self.oldheader._replace(
            structs=tuple(structs[i] for i in structrange)
        )
            ...
            if elemtype is None:
                for i in range(length):
                    ...
                    if g_ref.is_list or g_ref.is_reference:
                        ...
                        res._append(
                            self._defer_struct(
                                self.header.structs[g_ref.type], g_ref.address
                            ),
                            elem_offset,
                        )
                        ...
                        res._append(
                            self._defer_value(TYPES_BY_ID[g_ref.type], g_ref.address),
                            elem_offset,
                        )
                    elem_offset += Generic.size
            else:
                for i in range(length):
                    (address,) = UINT32.format[bigendian].unpack_from(
                        self._data, elem_offset
                    )
                    if address == 0xFFFFFFFF:
    ...
    def _read_value(self, datatype, offset):
        if datatype is ECString:
            (address,) = datatype.format[self._bigendian].unpack_from(
                self._data, offset
            )
            if address == 0xFFFFFFFF:
                return None
            ...
            label, address = datatype.format[self._bigendian].unpack_from(
                self._data, offset
            )
            if address == 0xFFFFFFFF:
            ...
            return datatype(
                *datatype.format[self._bigendian].unpack_from(self._data, offset)
            )
    ...
    def _read_string(self, offset, tlk=None):
        if self.use_cstring:
            try:
                ...
                raise IndexError(
                    "list index out of range", offset, len(self.stringcache)
                )
        else:
            try:
                s = self.stringcache[offset]
            except KeyError:
                (length,) = UINT32.format[self._bigendian].unpack_from(
                    self._data, offset
                )
    ...
    def _read_generic(self, offset):
        type_id, address = Generic.format[self._bigendian].unpack_from(
            self._data, offset
        )
    ...
    def tofile(self, f):
        ...
        def_align = 4
        data_section = bytearray(self._data)
        header = self.header
            ...
            for field in structure.fields:
                if changed or data.isloaded(field.label):
                    write_field(
                        ...
                        data[field.label],
                        offset + field.offset,
                        changed or data.ischanged(field.label),
                    )
        ...
        def write_list(field, data, offset, changed):
            if not data:
                ...
                if (
                    not in_data
                    or self._off2obj[address] is not data
                    or len(data) > data._original_size
                ):
                ...
                if data_changed or not in_data:
                    ...
                    elem_offsets = range(elem_start, elem_end, elem_size)
                    ...
                    if data.indirect:
                        if datatype is None:
                            ...
                            def write(datatype, item, elem_offset, elem_changed):
                                write_reference(
                                    datatype, item, elem_offset, elem_changed
                                )
                    ...
                    for elem_offset, orig_offset, item in zip(
                        elem_offsets, orig_offsets, data._list
                    ):
                        elem_changed = True
                        ...
                        if orig_offset is not None:
                            if orig_offset != elem_offset:
                                data_section[elem_offset : elem_offset + elem_size] = (
                                    self._data[orig_offset : orig_offset + elem_size]
                                )
                            elem_changed = False
            ...
            if datatype is ECString:
                if data is None:
                    datatype.format[bigendian].pack_into(
                        data_section, offset, 0xFFFFFFFF
                    )
                else:
                    datatype.format[bigendian].pack_into(
        ...
        def write_generic(data, offset, changed):
            ...
            if changed:
                if data is None:
                    ...
                    Generic.format[bigendian].pack_into(
                        data_section, offset, 0x40000000 | typeid, address
                    )
                    ...
                    if changed:
                        flags = pack_flags(
                            data.is_list, data.is_struct, data.is_reference
                        )
                        Generic.format[bigendian].pack_into(
        ...
        def write_reference(datatype, data, offset, changed):
            if changed:
                if data is None:
                    Reference.format[bigendian].pack_into(
                        data_section, offset, 0xFFFFFFFF
                    )
                else:
        ...
        header_section = bytearray(self._headerdata)
        ...
        if version == "V4.0":
            Header40Format[bigendian].pack_into(
                header_section,
                12,
                header.file_type,
                header.file_version,
            ...
            Header41Format[bigendian].pack_into(
                header_section,
                12,
                header.file_type,
                header.file_version,
